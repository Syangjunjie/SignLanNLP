...
by NLPIR (for that, see :mod:`sentence_segment.nlpir`). A few design choices have been
...
__version__ = '0.5.2'
logger = logging.getLogger('sentence_segment')

# 词性过滤
FILTER_POS = ('vhsi', 'q', 'uzhe', 'ule', 'ude1', 'ude2', 'ude3', 'usuo', 'udeng', 'uyy', 'udh', 'uls', 'uzhi', 'ulian',
              'e', 'y', 'o', 'x',
...
def open(data_dir=nlpir.PACKAGE_DIR, encoding=ENCODING,
    ...
    This calls the function :func:`~sentence_segment.nlpir.Init`.

    :param str data_dir: The absolute path to the directory that has NLPIR's
        `Data` directory (defaults to :data:`sentence_segment.nlpir.PACKAGE_DIR`).
...
def close():
    ...
    This calls the function :func:`~sentence_segment.nlpir.Exit`.
...
#
#     See :func:``sentence_segment.pos_map.get_pos_name`` for more information.
#
#     """
...
def segment(s):
    ...
    *pos_tagging* is `True`, then each item is a tuple (``(token, pos)``), e.g.
    ...
    This uses the function :func:`~sentence_segment.nlpir.ParagraphProcess` to segment
    *s*.
    ...
    s = _decode(s)
    s = s.strip()
    logger.debug("Segmenting text with{} POS tagging: {}.".format('', s))
    result = nlpir.ParagraphProcess(_encode(s), True)
    ...
    word_poses = []
    for token in tokens:
        word_pos = token.rsplit('/', 1)
        if len(word_pos) == 1:
            word_pos = [word_pos[0], None]
        # if word_pos[1] in FILTER_POS:
...
def get_key_words(s, max_words=50, weighted=False):
    ...
    *weight* is a float. If it's *False*, then each list item is a string.
    ...
    This uses the function :func:`~sentence_segment.nlpir.GetKeyWords` to determine
    the key words in *s*.
